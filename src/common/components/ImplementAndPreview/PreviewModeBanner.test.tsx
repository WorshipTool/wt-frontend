...
jest.mock('../../ui', () => ({
	Box: ({ children, onClick, onKeyDown, role, tabIndex }: { children: React.ReactNode; onClick?: () => void; onKeyDown?: React.KeyboardEventHandler; role?: string; tabIndex?: number }) => (
		<div {...(role ? { 'data-testid': 'banner', role, tabIndex } : {})} onClick={onClick} onKeyDown={onKeyDown}>{children}</div>
	),
	Tooltip: ({ children, title }: { children: React.ReactElement; title: string }) =>
		title ? <div data-testid="tooltip" data-tooltip={title}>{children}</div> : <>{children}</>,
}))
...
describe('PreviewModeBanner', () => {
	...
	it('renders with PR title when available', () => {
		...
		render(<PreviewModeBanner isAdmin onClick={jest.fn()} />)
	...
	it('calls onClick when banner is clicked', () => {
		...
		render(<PreviewModeBanner isAdmin onClick={onClick} />)
	...
	it('does not call onClick when not admin', () => {
		const onClick = jest.fn()
		render(<PreviewModeBanner isAdmin={false} onClick={onClick} />)
		fireEvent.click(screen.getByTestId('banner'))
		expect(onClick).not.toHaveBeenCalled()
	})

	it('wraps in tooltip when not admin', () => {
		render(<PreviewModeBanner isAdmin={false} onClick={jest.fn()} />)
		expect(screen.getByTestId('tooltip')).toBeInTheDocument()
	})

	it('does not wrap in tooltip when admin', () => {
		render(<PreviewModeBanner isAdmin onClick={jest.fn()} />)
